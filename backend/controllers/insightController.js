...
const buildMonthlyInsight = async (userId) => {
  const data = await pool.query(
    `WITH current_month AS (
        ...
        trend AS (
            ...
            (SELECT expenses FROM current_month) AS expenses,
  ...
  const content = await generateMonthlyInsight({
    ...
    previousMonths: (row.trend || []).map((m) => ({
      month: m.month,
      income: parseFloat(m.income),
      totalExpenses: parseFloat(m.expenses),
    })),
    currency,
  });
...
const buildBudgetAlert = async (userId, categoryId) => {
  if (!categoryId) {
    const err = new Error("Category ID required for budget alert.");
    err.status = 400;
    throw err;
  }
  ...
  if (budgetRow.rows.length === 0) {
    const error = new Error("Budget not found for the category");
    error.status = 404;
    throw error;
  }
...
export const generateInsight = async (req, res) => {
  ...
  try {
    ...
    if (type === "monthly_summary") {
      ...
      return res.status(400).json({
        message:
          "Invalid insight type or missing category ID for budget alerts.",
      });
    }

    const inserted = await pool.query(
      `INSERT INTO ai_insights (user_id, insight_type, content_json, period_start, period_end)
        VALUES($1, $2, $3, $4, $5)
        RETURNING *`,
      [req.userId, type, result.content, result.periodStart, result.periodEnd],
    );
